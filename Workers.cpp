...
void printWorker(Worker T[], int N) { // Зачем это в классе то????
	for (int i = 0; i < N; i++) {
		cout << setw(4) << left << T[i].getnum() << " " << setw(20) << left << T[i].getname() << " " << T[i].getyear() << " " << T[i].getsalary() << endl;
	}
}

void randWorker(Worker T[], int N) { // Зачем это в классе то????
...
Worker findRichest(Worker T[], int N) { // Зачем это в классе то????
	int richest = 0;
	for (int i = 1; i < N; i++) {
...
int averageSalary(Worker T[], int N) { // Зачем это в классе то????
	int tmp = 0;
	for (int i = 1; i < N; i++) {
		tmp += T[i].getsalary();
	}
...
void sortByAge(Worker T[], int N) { // Зачем это в классе то????
	bool sorted = false;
	while (!sorted) {
		sorted = true;
		for (int i = 0; i < N - 1; i++)
